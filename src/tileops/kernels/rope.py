...
def _make_rope_neox_position_ids_thd(
    ...
    copies the columns past it. The rotation itself runs in f32 and rounds once.
    """
    half = rotary_dim // 2
            ...
            with T.Kernel(T.ceildiv(n_walked, block_size), threads=threads_arg) as bx:
                for i, j in T.Parallel(threads_arg, npt_arg):
                    ...
                    if pair_idx < n_pairs:
                        ...
                        x_low = T.cast(x[low], "float32")
                        x_high = T.cast(x[low + half], "float32")
                        c = T.cast(cos_table[pos, col], "float32")
                        s = T.cast(sin_table[pos, col], "float32")
                        y[low] = T.cast(x_low * c - x_high * s, dtype)
                        y[low + half] = T.cast(x_high * c + x_low * s, dtype)
                if n_tail > 0:
                    for i, j in T.Parallel(threads_arg, npt_arg):
